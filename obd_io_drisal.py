...
from obd_sensors_drisal import hex_to_int
...
def decrypt_dtc_code(code):
    ...
    for i in range(0,3):
        ...
        tc = obd_sensors_drisal.hex_to_int(current[0]) #typecode
        tc = tc >> 2
        if   tc == 0:
            type = "P"
        elif tc == 1:
        ...
        dig1 = str(obd_sensors_drisal.hex_to_int(current[0]) & 3)
        dig2 = str(obd_sensors_drisal.hex_to_int(current[1]))
        dig3 = str(obd_sensors_drisal.hex_to_int(current[2]))
        dig4 = str(obd_sensors_drisal.hex_to_int(current[3]))
...
class OBDPort:
     ...
     def sensor(self , sensor_index):
         ...
         sensor = obd_sensors_drisal.SENSORS[sensor_index]
     ...
     def sensor_names(self):
         """Internal use only: not a public interface"""
         names = []
         for s in obd_sensors_drisal.SENSORS:
             names.append(s.name)
         return names

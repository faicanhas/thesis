...
def throttle_pos(code):				# Throttle Position
    code = hex_to_int(code)	
    code = code * 100.0 / 255.0
    return round(code, 2)			# membatasi 2 angka di belakang koma, edited by Faisal on 29 June 2016
...
def speed(code):					# Kecepatan
    code = hex_to_int(code)
    # return code / 1.609       	# default MPH, commented by Faisal on 27 June 2016
    return code                 	# KPH

def percent_scale(code):			# Engine Load
    code = hex_to_int(code)
    code = code * 100.0 / 255.0
    return round(code, 2)			# membatasi 2 angka di belakang koma, edited by Faisal on 29 June 2016
...
def temp(code):						# Coolant Temperature
    code = hex_to_int(code)
    c = code - 40 
    # return 32 + (9 * c / 5)   	# default Fahrenheit, commented by Faisal on 27 June 2016
    return c                    	# Celcius
